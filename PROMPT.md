**MINIONS CONTRACTS — CONTRACT MANAGER**

You are tasked with creating the complete initial foundation for **minions-contracts** — structured contract and obligation tracking. This is part of the Minions ecosystem, a universal structured object system designed for building AI-native tools.

---

**PROJECT OVERVIEW**

`minions-contracts` is a structured system for managing contracts, clauses, parties, obligations, deadlines, and risk flags. It tracks what you owe, what's owed to you, and when things expire.
...
1. **`contract`** — A legal agreement (Fields: `title` (string, required), `parties` (tags, required), `startDate` (date), `endDate` (date), `value` (number), `status` (select: draft/active/expired/terminated), `documentUrl` (url))
2. **`clause`** — A specific provision (Fields: `title` (string, required), `content` (textarea, required), `clauseType` (select: termination/liability/payment/confidentiality/ip)) — Relations: `part_of` → contract
3. **`party`** — An entity in a contract (Fields: `name` (string, required), `role` (select: buyer/seller/licensee/licensor), `contactEmail` (email)) — Relations: `references` → contract
4. **`obligation`** — A required action (Fields: `description` (textarea, required), `dueDate` (date), `responsibleParty` (string), `status` (select: pending/completed/overdue)) — Relations: `part_of` → contract
5. **`risk-flag`** — A potential issue (Fields: `description` (textarea, required), `severity` (select: low/medium/high), `category` (select: financial/legal/operational), `mitigation` (textarea)) — Relations: `references` → clause or contract
6. **`deadline`** — Key date (Fields: `name` (string, required), `date` (date, required), `type` (select: renewal/termination/review)) — Relations: `references` → contract
...
- Obligation tracking with deadline awareness
- Risk flag analysis across contract portfolio
- Version comparison between contract revisions
...
```bash
contracts expiring --days 30
contracts obligations --party acme
contracts diff <v1> <v2>
contracts risk-flags <id>
contracts summary <id>
```
...
Agents can monitor contract expiration, track obligations, flag risks, and compare contract versions automatically.
...
Complete Python port with identical functionality. Published to PyPI as `minions-contracts`.
